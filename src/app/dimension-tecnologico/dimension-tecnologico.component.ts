...
@Component({
  selector: 'app-dimension-tecnologico',
  templateUrl: './dimension-tecnologico.component.html',
  styles: []
})
export class DimensionTecnologicaComponent implements OnInit {
  ...
  onWriteResultado(numero: string) {
    sessionStorage['tecnologico'] = numero
    console.log("tecnologico: " + sessionStorage.getItem('tecnologico'));
  }
  finishTestResultado(){
    let resultado = Number(sessionStorage.getItem('tecnologico')) 
    sessionStorage.setItem('tecnologico-tipo', '');
    sessionStorage.setItem('graphTec', '');
    
    if(resultado <= 3){
      this.openDialog("EXPLORATORIO");
      sessionStorage['graphTec'] = 1;
    }
    if(resultado > 3 && resultado <= 6){
      this.openDialog("INTEGRADOR");
      sessionStorage['graphTec'] = 2;
    }
    if(resultado > 6 && resultado <= 9){
      this.openDialog("INNOVADOR");
      sessionStorage['graphTec'] = 3;
    }
  }
  openDialog(nivel: string): void {
    sessionStorage['tecnologico-tipo'] = nivel;
    sessionStorage['tipo'] = nivel;
    sessionStorage['tipoTecnologia'] = nivel;
    let element: HTMLElement = document.getElementsByClassName('btn')[0] as HTMLElement;
    element.click();
    sessionStorage['tipo'] = "EXPLORATORIO";
    sessionStorage['modulo2'] = '0';
  }
